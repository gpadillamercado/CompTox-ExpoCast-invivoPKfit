...
And this views the coefficients for row 5 (the same chemical, fitted with the 2-compartment model instead of the 1-compartment). Note that there are more coefficients, and they have different names, compared to the 1-compartment model.
...
**Note: If the flat model is the winning model, it should be considered a flag to investigate potential underlying problems with the CvT data, and/or possible model mis-specification — for example, dose-dependent kinetics not captured by the model.**
...
Data grouping instructions are given using a function called `facet_data()` (named by analogy with `ggplot2`). You specify data grouping using an *unqouted* variable names specified in the `mapping` argument to `pk()`. Whatever variables you specify, the data will be grouped by unique combinations of those variables.
...
`my_pk$pk_groups$nca_group` instructs `invivopkfit` on how to group data for calculation of summary statistics, including non-compartmental analysis (NCA).
Data will be split into groups according to unique combinations of the variables
specified in `nca_group`.
Then, non-compartmental analysis will be performed for each group using `PK::nca()`.
...
`my_pk$pk_settings$optimx` provides control settings for the optimizer used to fit the model(s) to the data, which is `optimx::optimx()`. It is the result of a call to `settings_optimx()` with default arguments. In other words, just calling `pk()` by itself is the same as
...
There is only one default argument: `error_group`. It is specified, just like data grouping in `facet_data()`, using unquoted variables.
...
- `param_groups`: A named list of character vectors that associate particular
parameters which users may want to optimize as a group.
...
### Simplified model modification interface

In some cases, re-writing an entirely new `pk_model` is necessary, but we
acknowledge that it may be sufficient to make smaller modifications to existing
models. As of version 2.0.2 of `invivoPKfit`, we have included functionality
to set optimized parameters or initial values with the
`set_params_optimize` and `set_params_start` functions.
Additionally, we recommend to give a unique model object name to modified models,
and pass this new model through `adjust_model_name` so that it is recognized by
`invivoPKfit`.

Here is an example of making a new 2-compartment model which only optimizes the
`k12` and `k21` rate parameters and gives the parameter `V1` a value of 1.
...
Another addition to this function family is `toggle_clearance_mode()`, which
allows users to change the default hepatic clearance assumption in
`model_httk_gas_pbtk`, which is restrictive clearance.
